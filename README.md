...
Validate the archive and the member metadata.

Validate the destination path.

Extract into a temporary file.

Verify the expected uncompressed size.

Verify the ZIP CRC32.

Atomically place the verified output.

Convert the archive to sparse storage when necessary.

Punch the processed compressed byte range.

Measure the resulting physical allocation.

Save resume state only after successful processing.
